...
<div class="wrap">
  ...
  <form method="post" action="options.php">
    ...
    <h2>Gravity Forms SMS Options</h2>

    <?php settings_fields( 'clockwork_gravityforms' ); ?>
    <?php do_settings_sections( 'clockwork_gravityforms' ); ?>
...
<table class="widefat fixed" cellspacing="0">
  ...
  <tbody class="list:user user-list">
    ...
    foreach( $this->forms as $form ) {
      ...
      $to = $meta['clockwork_to'];
      if( empty( $to ) ) {
        $opt = get_option( 'clockwork_gravityforms' );
        $to = $opt['default_to'];
      }
      $message = $meta['clockwork_message'];
    ?>
    <tr>
        <td style="padding: 10px 8px;"><p style="padding-top: 3px"><?php echo $form->title; ?></p></td>
